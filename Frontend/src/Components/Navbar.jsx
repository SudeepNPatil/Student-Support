...
export default function Navbar() {
  ...
  const logout = async () => {
    ...
    await fetch('https://student-support-s0xt.onrender.com/User/logout', {
      ...
      .then((data) => data.json())
      .then((data) => (console.log(data), setloadingmodal(false)));
  };

  return (
    <div
      onClick={(e) => (e.stopPropagation(), setisclicked(false))}
      className="flex justify-between h-20 border-b fixed bg-white w-full z-50"
    >
      <Link to={`/Home`} className="flex">
        <img
          src={logo}
          alt="logo"
          className="ml-2 rounded-full xl:ml-16 hover:text-[#FFD93D] lg:ml-10 md:ml-6 sm:ml-4  w-16 h-16 mt-2 hover:scale-125 duration-500 ease-in-out cursor-pointer"
        />
        <h1 className="mt-7 ml-1 md:text-xl lg:text-2xl xs:text-lg hover:text-[#FFD93D] font-bold lg:mt-6 lg:ml-2 hover:scale-105 duration-500 ease-in-out cursor-pointer">
          CODE MENTOR
        </h1>
      </Link>
        ...
        {isLogin ? (
          <div className="flex flex-row justify-between sm:gap-2 lg:gap-5 items-center">
            ...
            <HiOutlineDotsVertical
              ...
              className="md:-mr-10 lg:-mr-6 text-gray-600 hover:bg-yellow-100 rounded-full p-1 cursor-pointer"
            />
          </div>
        ) : (
      ...
      <div
        className={`mt-5 bg-white border rounded-md px-5 pb-3 fixed right-0 transform transition-transform duration-300 ease-in-out ${
          isopen ? 'translate-x-0' : 'translate-x-full'
        } `}
      >
        <RxCross2
          size={30}
          className="relative left-36 p-1"
        ...
        <div className="flex flex-col gap-4 w-auto scale-x-100 h-auto relative min-w-40 bg-white font-semibold rounded-sm">
          ...
          {isLogin ? (
            <div
              ...
              className="flex flex-row mt-2 hover:bg-blue-600 bg-[#00000004] justify-between px-2 py-2 rounded-xl items-center"
            >
      ...
      <button
        className="md:hidden border-2 bg-gray-50 rounded-md w-10 block h-10 text-center text-3xl pl-1 mr-4 mt-5"
