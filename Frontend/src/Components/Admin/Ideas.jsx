...
export default function Ideas() {
  ...
  useEffect(() => {
    fetch('https://student-support-s0xt.onrender.com/CustomBuildService/custom')
      .then((data) => data.json())
  ...
  return (
    <div className="flex flex-col flex-1 h-screen overflow-auto">
      <div className="flex felx-row justify-between px-14 border-b py-8">
        <h1 className="text-3xl font-bold ">Custom Builds</h1>
      ...
      <div className="flex flex-col flex-grow mx-14 my-14 rounded-t-2xl border h-auto ">
        <div className="flex flex-row justify-between items-center w-full  px-8 py-8 border-b">
          <div className="flex flex-row  items-center gap-5 ">
            ...
            <h1 className="text-2xl font-bold">All Custom Builds</h1>
          </div>
          <p className="text-2xl text-black font-bold bg-opacity-75">
            total Custom builds :{' '}
            <span className="text-green-500">{Customs.length}</span>
          </p>
        </div>
        {Customs.length > 0 ? (
          <table className="text-center border-collapse">
            <tr className="h-20">
              ...
              <th className="border w-60">User info</th>
              <th className="border w-28">Date & Time</th>
              <th className="border w-80">description</th>
              ...
              return (
                <tr key={index} className="border h-20">
                  ...
                  <td className="flex flex-col items-start justify-center px-10 gap-1 py-4">
                    <p className="text-[18px]">{Custom.name}</p>
                    <p className="text-sm">{Custom.unic}</p>
                    <p className="text-sm">{Custom.Phnumber}</p>
                  </td>
                  <td className="border">
                    <span className="block text-center">
                      {Custom.createdAt.slice(0, 10)}
                    </span>
                    <span className="block ">
                      {Custom.createdAt.slice(11, 19)}
                    </span>
                  </td>
                  <td className="border ">
