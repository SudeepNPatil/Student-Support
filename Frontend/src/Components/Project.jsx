...
import { ShoppingBag, Wifi, BrainCircuit, BarChart2, UserCircle2, LayoutGrid } from "lucide-react";

const categories = [
  { label: "E-commerce", to: "/Project/ecommerce", icon: <ShoppingBag size={18} /> },
  { label: "IOT", to: "/Project/IOT", icon: <Wifi size={18} /> },
  { label: "AI/ML", to: "/Project/AI_ML", icon: <BrainCircuit size={18} /> },
  { label: "Business DashBoards", to: "/Project/business", icon: <BarChart2 size={18} /> },
  { label: "Portfolio", to: "/Project/Portfolio", icon: <UserCircle2 size={18} /> },
  { label: "Others", to: "#", icon: <LayoutGrid size={18} /> },
];
    ...
    <>
      ...
      <div className="hidden inset-0 sm:flex xl:flex-col sm:flex-row flex-wrap">
        <div className="inset-0 hidden sm:flex xl:flex-row sm:flex-col flex-wrap xl:pt-20 sm:pt-20 overflow-x-scroll no-scrollbar">
          ...
          {/* Sidebar */}
          <div className="xl:w-[20%] sm:w-full xl:gap-0 xl:px-0 lg:px-12 md:px-8 sm:px-7 sm:gap-10 overflow-x-scroll no-scrollbar shadow-md sm:flex xl:flex-col sm:flex-row xl:pb-20 sm:pb-5">
            ...
            {categories?.map(({ label, to, icon }) => (
              <Link
                key={label}
                to={to}
                className="group flex items-center pr-2 pl-10 py-2 gap-2 text-center mt-5 font-semibold opacity-80 min-w-52 cursor-pointer transition-colors duration-200 hover:bg-yellow-100 hover:opacity-100"
              >
                <span className="text-gray-600 group-hover:text-yellow-600 transition-colors duration-200">{icon}</span>
                {label}
              </Link>
            ))}
          </div>
          ...
          <div className="flex-1">
            <div className="flex xl:px-20 lg:px-12 md:px-10 sm:px-9 sm:justify-between h-20 shadow-[0_2px_4px_-2px_rgba(0,0,0,0.3)] items-center">
              ...
              <div className="relative">
                ...
                {Cartitem.length > 0 && <span className="bg-pink-500 text-black font-semibold text-center absolute -top-3 -right-2 rounded-full w-6 h-6">{Cartitem.length}</span>}
              </div>
              <div className="relative">
                <Link to={`/Project/Wishlist`}><FaRegHeart className="w-7 h-7 cursor-pointer" /></Link>
                {WishlistItem.length > 0 && <span className="bg-pink-500 text-black font-semibold text-center absolute -top-3 -right-2 rounded-full w-6 h-6">{WishlistItem.length}</span>}
              </div>
              <div className="relative">
                <Link to={`/Project/Order`}><BsTruck className="w-7 h-7 cursor-pointer" /></Link>
                {Order.length > 0 && <span className="bg-pink-500 text-black font-semibold text-center absolute -top-3 -right-2 rounded-full w-6 h-6">{Order.length}</span>}
              </div>
            </div>
            <Outlet />
          </div>
      ...
      <div className="sm:hidden w-screen h-auto pt-20" onClick={(e) => { e.stopPropagation(); setstretch(false); }}>
        <div className="flex justify-between items-center pt-5 pb-3 px-2 gap-2">
          <button className="w-2/3 min-w-[50px] max-w-[110px] h-8 border text-sm opacity-85 rounded-lg text-center bg-green-100" onClick={(e) => { e.stopPropagation(); setstretch(true); }}>
            Category<IoArrowForwardCircleOutline size={14} className="inline-block ml-0.5" />
          </button>

          <Link to={`/Project/Search`} className="relative">
            <BsSearch className="text-sm xss:text-base absolute left-2 top-2" />
            <input type="text" name="text" id="text" className="pl-8 border rounded-md h-8 w-full" placeholder="Search..." onChange={(e) => setsearchProduct(e.target.value)} />
          </Link>
          ...
          <div className={`bg-green-50 rounded-md h-full z-10 fixed right-0 top-24 w-auto transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="relative">
              ...
              {Cartitem.length > 0 && <span className="bg-pink-500 text-black font-semibold text-center absolute text-[10px] top-0 left-8 rounded-full px-1.5">{Cartitem.length}</span>}
            </div>
            <div className="relative">
              ...
              {WishlistItem.length > 0 && <span className="bg-pink-500 text-black font-semibold text-center absolute rounded-full text-[10px] top-0 left-8 px-1.5">{WishlistItem.length}</span>}
            </div>
            <div className="relative">
              ...
              {Order.length > 0 && <span className="bg-pink-500 text-black font-semibold text-center absolute rounded-full text-[10px] top-0 left-8 px-1.5">{Order.length}</span>}
            </div>
          </div>
        </div>
        ...
        <div className={`bg-green-50 flex flex-col z-10 px-10 rounded-xl h-screen w-[85%] xss:w-[75%] xs:w-[70%] ls:w-[65%] sl:w-[60%] absolute left-0 top-24 transform transition-transform duration-300 ease-in-out ${stretch ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          {categories?.map(({ label, to, icon }) => (
            <Link
              key={label}
              to={to}
              className="group flex items-center gap-2 justify-center text-center mt-5 border opacity-80 min-w-20 cursor-pointer rounded-md bg-gray-200 pt-1 pb-1 transition-colors duration-200 hover:bg-yellow-100 hover:opacity-100"
            >
              <span className="text-gray-600 group-hover:text-yellow-600 transition-colors duration-200">{icon}</span>
              {label}
            </Link>
